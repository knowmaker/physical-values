...
document.addEventListener(
  ...
  (event) => {
    if (event.button !== 2) {
      ...
      gkmenu.classList.remove("active");
    }
  },
  false
);
...
document.addEventListener("keydown", (event) => {
  if (event.keyCode == 27) {
    ...
    if (currentShownLaw.hexes) {
      hexagons = currentShownLaw.hexes.map((hex) =>
...
function rememberHexagon(hex) {
  ...
  if (clickedHexagons.length == 3) {
    ...
    if (checkParallelogram(tempclickedHexagons)) {
      clickedHexagonsCoords = tempclickedHexagons.map((hexagon) =>
      ...
      currentLaw = {
        ...
        id: hexDataLaw
          .map((hex) => `${hex.LT}${hex.GK}`)
          .sort()
          .join(""),
      };
      document.getElementById("lawConfig").innerHTML =
        currentLaw.equationInTerms;
      document.getElementById("lawFormula").innerHTML =
        currentLaw.equationInLetters;
  ...
  if (clickedHexagons.length == 4) {
    if (checkParallelogram(clickedHexagons)) {
      clickedHexagonsCoords = clickedHexagons.map((hexagon) =>
        getHexCanvasCoords(hexagon)
      );
...
function finRedact() {
  ltInput = replacePowNumbersBtoS(
    `L${getFromInput("LLT")}T${getFromInput("TLT")}`
  );
  gkInput = replacePowNumbersBtoS(
    `G${getFromInput("GGK")}K${getFromInput("KGK")}`
  );
...
function addLaw() {
  ...
  if (data.laws[currentLaw.type][currentLaw.id] === undefined) {
    ...
    alert("Данный закон уже существует");
  }
  currentLaw = {};
}
...
function showLaws() {
  ...
  change = {
    force: "Силовые соотношения",
    mech: "Законы механики",
    magn: "Магнитное поле",
    gravity: "Гравитационные законы",
    electro: "Электромагнитные закономерности",
    kkv: "Соотношения ККВ",
  };
...
fileSelector.addEventListener("change", (event) => {
  ...
  document.getElementById("sota_filenm").value = fileList[0].name.replace(
    ".jsota",
    ""
  );
});
